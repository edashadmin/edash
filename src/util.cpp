...
const char * const BITCOIN_CONF_FILENAME = "edash.conf";
const char * const BITCOIN_PID_FILENAME = "edashd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "edash" is a composite category enabling all EDash-related debug output
            if(ptrCategory->count(string("edash"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EDashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EDashCore
    // Mac: ~/Library/Application Support/EDashCore
    // Unix: ~/.edashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EDashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EDashCore";
#else
    // Unix
    return pathRet / ".edashcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty edash.conf if it does not excist
