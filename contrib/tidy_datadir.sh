...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete EDash database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no EDash datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old EDash datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected EDash 0.7 datadir."
    ;;
  3)
    echo "Detected EDash pre-0.8 datadir."
    ;;
  4)
    echo "Detected EDash 0.8 datadir."
    ;;
esac
